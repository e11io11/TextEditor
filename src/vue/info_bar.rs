...
use super::{char_size, str_rect, BAR_COLOR, OUTLINE_COLOR, TEXT_COLOR};
...
impl InfoBar {
    ...
    pub fn draw_bar(&self, cursor: (usize, usize), canvas: &mut Canvas<Window>, font: &Font) {
        canvas.set_draw_color(BAR_COLOR);
        canvas.fill_rect(self.area).unwrap();
        ...
        let surface = font.render(&cursor_str).blended(TEXT_COLOR).unwrap();
    ...
    pub fn refresh(&self, cursor: (usize, usize), canvas: &mut Canvas<Window>, font: &Font) {
